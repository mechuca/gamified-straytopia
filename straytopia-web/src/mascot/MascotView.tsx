...
export function MascotView({
  ...
  if (compact) {
    return (
      <div style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '14px 16px', backgroundColor: COLOR.surface, borderRadius: 20, border: `2px solid ${COLOR.hairline}` }}>
        <BuddyMascot scene={scene} size={56} prefersReducedMotion={prefersReducedMotion} />
        <div style={{ flex: 1 }}>
          <div style={{ fontFamily: 'Fredoka, sans-serif', fontWeight: 600, fontSize: 15, color: COLOR.ink, lineHeight: 1.3 }}>{finalMessage}</div>
          {finalSub && <div style={{ fontFamily: 'Nunito, sans-serif', fontWeight: 500, fontSize: 13, color: COLOR.muted, marginTop: 2 }}>{finalSub}</div>}
        </div>
        {finalAction && onAction && (
          <button onClick={onAction} style={{ fontFamily: 'Fredoka, sans-serif', fontWeight: 600, fontSize: 14, color: COLOR.jungle, background: 'none', border: 'none', cursor: 'pointer', padding: '4px 8px' }}>{finalAction}</button>
        )}
      </div>
    );
  }
    ...
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 12, padding: '16px 0' }}>
      ...
      <AnimatePresence>
        {showBubble && finalMessage && (
          ...
          >
            <div style={{ fontFamily: 'Fredoka, sans-serif', fontWeight: 600, fontSize: 15, color: COLOR.ink, lineHeight: 1.3 }}>{finalMessage}</div>
            {finalSub && <div style={{ fontFamily: 'Nunito, sans-serif', fontWeight: 500, fontSize: 12, color: COLOR.muted, marginTop: 4 }}>{finalSub}</div>}
          </motion.div>
        )}
      </AnimatePresence>
      {finalAction && onAction && (
