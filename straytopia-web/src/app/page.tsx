    ...
    <div style={{ position: 'absolute', inset: 0, pointerEvents: 'none', overflow: 'hidden' }}>
      {Array.from({ length: 50 }).map((_, i) => (
        <motion.div
          ...
          // Use a viewport-relative fall distance and per-piece variation so confetti doesn't "land" in a straight line.
          animate={{ y: `calc(110vh + ${(i % 7) * 28}px)`, opacity: [0, 0.6, 0], rotate: 720 }}
          transition={{ duration: 1.8, delay: (i % 10) * 0.08, ease: [0.22, 1, 0.36, 1], times: [0, 0.25, 1] }}
          style={{ position: 'absolute', left: `${(i * 137) % 100}%`, top: 0, width: 10, height: 14, borderRadius: 2, backgroundColor: colors[i % colors.length] }}
        />
      ))}
    </div>
  );
    ...
    <div style={{ position: 'fixed', left: 0, right: 0, bottom: 0, zIndex: 50 }}>
      <div style={{ maxWidth: 500, margin: '0 auto', padding: `10px 12px calc(${safeBottom} + 10px)` }}>
        <div style={{ position: 'relative' }}>
          <div style={{ height: 74, backgroundColor: withOpacity(C.navBackground, 0.92), backdropFilter: 'blur(18px)', borderRadius: 26, border: `1px solid ${withOpacity(C.border, 0.9)}`, display: 'grid', gridTemplateColumns: 'repeat(4, 1fr) 76px', alignItems: 'center', padding: '8px 10px', boxShadow: `0 14px 34px ${withOpacity(C.shadow, 0.14)}` }}>
          {tabs.map((t) => {
